...
def population_constraint
  ...
  FROM countries
  WHERE population < (SELECT population FROM countries WHERE name = 'Poland') AND population > (SELECT population FROM countries WHERE name = 'Canada')
  SQL
end
...
def sparse_continents
  ...
  FROM countries
  WHERE population < 25000000 AND continent IN (SELECT continent
  FROM countries
  GROUP BY continent
